...
func RunLogin(ctx context.Context, store *AuthStore) (*Auth, error) {
	...
	authorizeURL := AuthorizeURL(redirectURI, challenge, state)
	...
	auth, err := ExchangeCode(ctx, code, redirectURI, verifier)
	if err != nil {
		return nil, err
	}
