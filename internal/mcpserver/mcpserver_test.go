...
func TestReferenceUploadRegistrationIsHostedOnly(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{name: "hosted", server: New(true, generate, usage, upload, nil, nil), want: true},
		{name: "stdio", server: New(false, generate, usage, upload, nil, nil), want: false},
		{name: "hosted without storage", server: New(true, generate, usage, nil, nil, nil), want: false},
	} {
...
func TestReferenceUploadRegisteredToolCallsHostedHandler(t *testing.T) {
	...
	server := New(true, generate, usage, HostedReferenceUpload(issuer), nil, nil)
	serverTransport, clientTransport := mcp.NewInMemoryTransports()
